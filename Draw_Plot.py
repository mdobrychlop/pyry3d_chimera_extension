...
class Log_Diagram:
    def draw_diagram(self,logfile):
        ...
        self.overall, self.clashes, self.restraints, self.mapfreespace, self.outbox, self.density, self.symmetry, self.chi, self.rg=self.reverse_diagram(logfile)
	...
	plt.plot(self.symmetry)
        ...
        plt.legend(("overall score","clashes","restraints","map free space","out box","density filling","symmetry"),loc=4)
    ...
    def reverse_diagram(self,logfile):
        score,clashes,restraints,mapfreespace,outbox,density,symmetry,chi,rg=self.prepare_values(logfile)
        s=[];c=[];r=[];m=[];o=[];d=[];sy=[];ch=[];rg=[];
        for i in score:
            #i=i*-1
            s.append(i)
        for i in clashes:
    ...
    def prepare_values(self,logfile):
        ...
        if len(lines[-1].split(" ")) > 5:
            if lines[-1].split(" ")[5].startswith("is") or lines[-1].startswith("cx") == False:
	...
	skip_first = 0 #BARDZO NIEELEGANCKIE, POPRAWIC JAK BEDZIE DOBRZE SIE GENEROWAL LOG
        ...
        while o==1:
            ...
            if re.search("^Number of rejected complexes.*",a):
                o=o+1
            if re.search("^cx score.*",a) != None:
                b=a.split(" ")
                if b[5].startswith("is") == False:
		    if skip_first == 1:
			niter = int(b[4])
        ...
        list_list = [slist, clist, rlist, elist, olist, dlist, sylist, chilist, rglist]
